...
class SolarMaskDataset(Dataset):
    ...
    def __read_data__(self):
        ...
        train_split = int(0.70 * total_records)  # 70% for training
        val_split = int(0.1 * total_records)  # 10% for validation
        print(
        ...
        if self.timeenc == 0:
            ...
            print(f'debug: shape before dropping "date" = {df_stamp.shape}')
        ...
        # new_features = self._add_new_time_features(df_stamp_ori)
        # data_stamp = np.concatenate((data_stamp, new_features), axis=1)

        # Concatenate the new features with the original data_x and data_y
    ...
    def _add_new_time_features(self, df_stamp):
        # Extract time components
        df_stamp["hour"] = df_stamp["date"].dt.hour
        df_stamp["day"] = df_stamp["date"].dt.day
        df_stamp["dayofweek"] = df_stamp["date"].dt.dayofweek
        df_stamp["month"] = df_stamp["date"].dt.month
        df_stamp["dayofyear"] = df_stamp["date"].dt.dayofyear
        ...
        df_stamp["dayofweek_sin"] = np.sin(2 * np.pi * df_stamp["dayofweek"] / 7)
        df_stamp["dayofweek_cos"] = np.cos(2 * np.pi * df_stamp["dayofweek"] / 7)
        df_stamp["month_sin"] = np.sin(2 * np.pi * df_stamp["month"] / 12)
        df_stamp["month_cos"] = np.cos(2 * np.pi * df_stamp["month"] / 12)
        df_stamp["dayofyear_sin"] = np.sin(2 * np.pi * df_stamp["dayofyear"] / 365)
        df_stamp["dayofyear_cos"] = np.cos(2 * np.pi * df_stamp["dayofyear"] / 365)

        # Select new feature columns
        new_features = df_stamp[
            [
                "hour_sin",
                "hour_cos",
                "day_sin",
...
#     def __read_data__(
#         self,
#         irradiance_column="辐照仪 10米全辐照（W/m²）",
#         threshold=5,
#         window_size=3,
#     ):
#         df_raw = pd.read_csv(os.path.join(self.root_path, self.data_path))
...
#         assert (
#             df_raw.columns[-1] == self.target
#         ), f"Last column must be target {self.target}"

#         cols_data = df_raw.columns[1:]  # Assume first column is date or non-feature
#         df_data = df_raw[cols_data]

#         # Generate y_mask
#         y_mask = np.ones(len(df_data))

#         # Mask condition Nan values 1: Y_t
#         y_nan_mask = np.isnan(df_raw.iloc[:, -1])
#         y_mask[y_nan_mask] = 0

#         # Apply sliding window to irradiance column
#         irradiance_values = df_data[irradiance_column].values

#         for i in range(window_size - 1, len(irradiance_values)):
#             window_sum = np.sum(irradiance_values[i - window_size + 1 : i + 1])
#             if window_sum < threshold:
#                 irradiance_values[i] = 0

#         df_data[irradiance_column] = irradiance_values

#         # Generate x_zero_mask
#         x_zero_mask = (df_data[irradiance_column] == 0).astype(int)

#         combined_mask = y_mask * (1 - x_zero_mask)

#         self.y_null_mask = combined_mask[border1:border2].to_numpy()

#         if self.scale_x_flag or self.scale_y_flag:
#             # Initialize scalers
#             if self.scale_x_flag:
#                 if self.scaler_type == "standard":
#                     self.scaler_x = StandardScaler()
...
#             filtered_train_data_x = train_data_x[train_data_x[irradiance_column] != 0]
#             filtered_train_data_y = train_data_y[train_data_y.iloc[:, 0] != 0]

#             # Fit scalers on training data
#             if self.scale_x_flag:
#                 self.scaler_x.fit(filtered_train_data_x.values)
#             if self.scale_y_flag:
#                 self.scaler_y.fit(filtered_train_data_y.values)

#             # Transform the entire dataset
#             if self.scale_x_flag:
#                 df_data_x_scaled = self.scaler_x.transform(df_data.iloc[:, :-1].values)
#             else:
#                 df_data_x_scaled = df_data.iloc[:, :-1].values

#             if self.scale_y_flag:
#                 df_data_y_scaled = self.scaler_y.transform(df_data.iloc[:, -1:].values)
#             else:
#                 df_data_y_scaled = df_data.iloc[:, -1:].values

#             # Combine scaled X and Y back into a single dataset
#             data = np.hstack((df_data_x_scaled, df_data_y_scaled))

#         else:
#             data = df_data.values

#         df_stamp = df_raw[["时间"]][border1:border2]
#         df_stamp["时间"] = pd.to_datetime(df_stamp["时间"])

#         if self.timeenc == 0:
#             # not used here
#             df_stamp["month"] = df_stamp["时间"].apply(lambda row: row.month)
#             df_stamp["day"] = df_stamp["时间"].apply(lambda row: row.day)
#             df_stamp["weekday"] = df_stamp["时间"].apply(lambda row: row.weekday())
#             df_stamp["hour"] = df_stamp["时间"].apply(lambda row: row.hour)
#             print(f'debug: shape before dropping "时间" = {df_stamp.shape}')
#             data_stamp = df_stamp.drop(["时间"], axis=1).values

#         elif self.timeenc == 1:
#             # used here
#             data_stamp = time_features(
#                 pd.to_datetime(df_stamp["时间"].values), freq=self.freq
#             )
#             data_stamp = data_stamp.transpose(1, 0)

#         df_stamp_ori = df_raw[["时间"]].copy()
#         df_stamp_ori["时间"] = pd.to_datetime(df_stamp_ori["时间"])

#         new_features = self._add_new_time_features(df_stamp_ori)

#         # Concatenate the new features with the original data_x and data_y
#         data_combined = np.concatenate(
#             (data[:, :-1], new_features.values, data[:, -1:]), axis=1
#         )

#         self.data_x = data_combined[border1:border2, :-1]
#         self.data_y = data_combined[border1:border2, -1:]
#         self.data_stamp = data_stamp
#         self.time_stamp = df_stamp_ori[border1:border2]["时间"]
...
#         y_null_mask = self.y_null_mask[r_begin:r_end]

#         return seq_x, seq_y, seq_x_mark, seq_y_mark, y_null_mask
...
#     def _add_new_time_features(self, df_stamp):
#         # Extract time components
#         df_stamp["hour"] = df_stamp["时间"].dt.hour
#         df_stamp["day"] = df_stamp["时间"].dt.day
#         df_stamp["dayofweek"] = df_stamp["时间"].dt.dayofweek
#         df_stamp["month"] = df_stamp["时间"].dt.month
#         df_stamp["dayofyear"] = df_stamp["时间"].dt.dayofyear
